<?php 	
	...
	$sql = 'SELECT * FROM recruitment ORDER BY s_no desc LIMIT 10';
...
?>
    	...
    	<div class=table-box>
        <br>
        <h2 id="heading1">CAREERS @ GGSIPU</h2>
        <br>
...
<?php
		$sql = "SELECT * FROM tender_notice WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
...
<?php
            $sql = "SELECT * FROM tender_notice WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
