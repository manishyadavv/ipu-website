<?php 	
	...
	$sql = 'SELECT * FROM notices_circulars ORDER BY s_no desc LIMIT 10';
...
<?php
		$sql = "SELECT * FROM notices_circulars WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
		$y=0;
		while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
		{
