<?php 	
	...
	$sql = 'SELECT * FROM ucdms_notices ORDER BY s_no desc LIMIT 10';
...
?>
    	...
    	<div class=table-box>
        <br>
        <h2 id="heading1">CENTRE FOR DISASTER MANAGEMENT STUDIES</h2>
        <br>
	<center><h3><a href="http://ipu.ac.in/uscdm/syllMBADM131212.pdf">Scheme/Syllabus</h2></center>
...
<?php
		$sql = "SELECT * FROM ucdms_notices WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
		$y=0;
		while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
		{
...
<?php
            $sql = "SELECT * FROM ucdms_notices WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
