<?php 	
	...
	$sql = 'SELECT * FROM datesheet ORDER BY s_no desc LIMIT 10';
...
<?php
		$sql = "SELECT * FROM datesheet WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
		$y=0;
		while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
		{
...
<?php
            $sql = "SELECT * FROM datesheet WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
