<?php 	
	...
	$sql = 'SELECT * FROM estates ORDER BY s_no desc LIMIT 10';
...
?>
    	...
    	<div class=table-box>
        <br>
        <h2 id="heading1">ESTATE BRANCH</h2>
		<br>
  ...
  <tr>
    <td >Sh. A. K. Verma <br />
      Joint Registrar</td>
    <td style="text-align:right">+91-11-25302245 
<br />
Email:estate@ipu.ac.in</td>
    </tr>
...
<?php
		$sql = "SELECT * FROM estates WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
		$y=0;
		while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
		{
...
<?php
            $sql = "SELECT * FROM estates WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
