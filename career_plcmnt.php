<?php 	
	...
	$sql = 'SELECT * FROM placementcell ORDER BY s_no desc LIMIT 10';
	$retval = mysql_query( $sql, $conn );
	
?>

    	<div class=table-box>
        <h2 id="heading1">CENTRALIZED CAREER GUIDANCE AND PLACEMENT CELL</h2>
	<p align="center" ><a href="http://www.ipu.ac.in/ccgpc/cgplacecordinators.pdf">USS Placement Coordinators</a> | <a href="http://www.ipu.ac.in/ccgpc/ntplc070114.pdf">Invitation to Corporate Sector</a></p>
        <br>
...
<?php
		$sql = "SELECT * FROM placementcell WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
...
<?php
            $sql = "SELECT * FROM placementcell WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
