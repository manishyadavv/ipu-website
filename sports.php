<?php 	
	...
	$sql = 'SELECT * FROM sports ORDER BY s_no desc LIMIT 10';
...
<?php
		$sql = "SELECT * FROM sports WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
...
<?php
            $sql = "SELECT * FROM sports WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
