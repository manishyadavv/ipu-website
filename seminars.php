<?php 	
	...
	$sql = 'SELECT * FROM seminar ORDER BY s_no desc LIMIT 10';
...
?>
    	...
    	<div class=table-box>
        <br>
        <h2 id="heading1">SEMINARS/CONFERENCE</h2>
        <br>
	<h3><small>*Click on the title to download the pdf</small></h3>
	<table>
    	<thead>
    	<tr>        
        	<th class="table-title">Topic</th>
		<th class="table-title">Event Date</th>
...
?>
	...
	<td><?php print $row['event_dates']; ?></td>        
...
<?php
		$sql = "SELECT * FROM seminar WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." ORDER BY s_no desc" ;
		$retval = mysql_query( $sql, $conn );
		$y=0;
		while($row = mysql_fetch_array($retval, MYSQL_ASSOC))
		{
...
<?php
            $sql = "SELECT * FROM seminar WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no desc" ;
            $retval = mysql_query( $sql, $conn );
